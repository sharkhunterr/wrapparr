...
const Tag=({accent})=>
  <div style={{color:accent,fontSize:9,letterSpacing:"0.3em",fontFamily:"JetBrains Mono,monospace",textTransform:"uppercase",marginBottom:7,opacity:.8}}>XENOCLOUD · 2024</div>
const Pill=({children,accent})=>
  <span style={{display:"inline-block",padding:"3px 10px",borderRadius:18,background:`${accent}1e`,border:`1px solid ${accent}38`,color:accent,fontSize:10,fontFamily:"Nunito,sans-serif"}}>{children}</span>
const VsB=({value,label="vs 2023"})=>{const p=value>0;return(
  <span style={{display:"inline-flex",alignItems:"center",gap:4,padding:"4px 11px",borderRadius:18,
    ...
    color:p?"#4ade80":"#f87171",fontSize:11,fontWeight:600,fontFamily:"Nunito,sans-serif",animation:"badge-p .5s ease .6s both"}}>
    {p?"↑":"↓"} {p?"+":""}{value}% {label}
  </span>
)}
function BigNum({value,suffix="",accent,active,delay=.05}){
  const v=useCounter(value,active)
  return <div style={{fontSize:"clamp(24px, 7vw, 42px)",fontWeight:800,color:accent,lineHeight:1,fontFamily:"Nunito,sans-serif",
        ...
        <div key={u.n} style={{display:"flex",alignItems:"center",gap:7,animation:`slide-up .4s ease ${.08+i*.07}s both`}}>
          ...
          <div style={{flex:1}}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:2}}>
              ...
              <span style={{fontSize:10,color:isMe?accent:"rgba(255,255,255,.3)",fontFamily:"JetBrains Mono,monospace"}}>{u.v.toLocaleString("fr-FR")}{unit}</span>
            </div>
    ...
    <PokeCard accent={accent} style={style}>
      <div style={{display:"flex",gap:10,padding:"10px 12px"}}>
        ...
        <div style={{flex:1,minWidth:0}}>
          <div style={{color:accent,fontSize:8,fontFamily:"JetBrains Mono,monospace",letterSpacing:"0.2em",textTransform:"uppercase",marginBottom:3}}>#{rank} · 2024</div>
          <div style={{color:"white",fontWeight:800,fontFamily:"Nunito,sans-serif",fontSize:11,lineHeight:1.25,marginBottom:5,overflow:"hidden",display:"-webkit-box",WebkitLineClamp:2,WebkitBoxOrient:"vertical"}}>{item.t}</div>
          {item.author&&<div style={{color:"rgba(255,255,255,0.38)",fontSize:9,fontFamily:"Nunito,sans-serif",marginBottom:4}}>{item.author}</div>}
...
function SlideCat({accent,bg,icon,label,sub}){
  return(
    <div style={{width:"100%",height:"100vh",display:"flex",flexDirection:"column",
      ...
      {/* Icon */}
      <div style={{fontSize:"clamp(40px, 12vw, 72px)",marginBottom:18,filter:`drop-shadow(0 0 48px ${accent}90)`,
        animation:"cat-icon-in 0.8s cubic-bezier(0.34,1.56,0.64,1) 0.1s both"}}>
        {icon}
      </div>

      {/* Label */}
      <h1 style={{fontSize:"clamp(24px, 8vw, 48px)",fontWeight:800,color:"white",fontFamily:"Nunito,sans-serif",
      ...
      {/* Subtitle */}
      <div style={{fontSize:15,color:`${accent}cc`,fontFamily:"Nunito,sans-serif",
      ...
      <div style={{position:"absolute",bottom:"11%",left:"50%",transform:"translateX(-50%)",
        fontSize:9,color:"rgba(255,255,255,0.2)",fontFamily:"JetBrains Mono,monospace",letterSpacing:".3em",
...
function PodiumPlatform({item,rankIdx,accent,revealed,isOne,statLabel,statValue}){
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",opacity:revealed?1:0,transition:"opacity .1s",flex:isOne?1.15:1}}>
      ...
      {revealed&&<div style={{fontSize:isOne?38:28,fontWeight:800,marginBottom:6,fontFamily:"Nunito,sans-serif",color:accent,lineHeight:1,
      ...
      <div style={{width:"100%",borderRadius:"6px 6px 0 0",
        ...
        {revealed&&<>
          <div style={{color:"white",fontWeight:700,fontFamily:"Nunito,sans-serif",fontSize:isOne?11:9,textAlign:"center",lineHeight:1.2,marginBottom:4,animation:"stat-count .4s ease .3s both"}}>
            {item.t.length>16?item.t.slice(0,14)+"…":item.t}
          </div>
          <div style={{color:accent,fontWeight:800,fontFamily:"JetBrains Mono,monospace",fontSize:isOne?15:11,animation:"stat-count .4s ease .45s both"}}>
            {statValue}
          </div>
          <div style={{color:"rgba(255,255,255,0.4)",fontSize:8,fontFamily:"Nunito,sans-serif",textTransform:"uppercase",letterSpacing:"0.1em",animation:"stat-count .4s ease .55s both"}}>
            {statLabel}
          </div>
        </>}
      </div>
  ...
  return(
    <div style={{width:"100%",height:"100vh",display:"flex",flexDirection:"column",
      ...
      <div style={{textAlign:"center",marginBottom:phase>=2?16:24,position:"relative",zIndex:5,transition:"margin .4s"}}>
        ...
        <div style={{fontSize:9,color:accent,letterSpacing:".3em",fontFamily:"JetBrains Mono,monospace",textTransform:"uppercase",marginBottom:4}}>XENOCLOUD · 2024</div>
        <h2 style={{fontSize:28,fontWeight:800,color:"white",fontFamily:"Nunito,sans-serif",lineHeight:1.05,animation:"slide-up .7s ease .2s both"}}>
          {title}
        </h2>
      </div>
      ...
      {phase===1&&(
        <div style={{zIndex:5,textAlign:"center",maxWidth:320,position:"relative"}}>
          ...
          <div style={{
            fontSize:18,color:"white",fontFamily:"Nunito,sans-serif",fontWeight:400,
          ...
          {revealed[2]&&data[3]&&(
            <div style={{display:"flex",alignItems:"center",gap:10,marginTop:12,padding:"8px 14px",borderRadius:10,background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.07)",animation:"slide-up .4s ease .2s both"}}>
              ...
              <div>
                ...
                <div style={{color:"white",fontWeight:600,fontFamily:"Nunito,sans-serif",fontSize:12}}>{data[3].t}</div>
              </div>
              <div style={{marginLeft:"auto",color:accent,fontFamily:"JetBrains Mono,monospace",fontSize:11}}>{data[3][statKey]}{statSuffix}</div>
            </div>
          )}
        </div>
      )}
...
function SlideIntro({accent,onStart}){
  return <div style={{textAlign:"center",maxWidth:380,width:"100%"}}>
    ...
    <div className="s0" style={{fontSize:9,color:accent,letterSpacing:".35em",fontFamily:"JetBrains Mono,monospace",textTransform:"uppercase"}}>XENOCLOUD · RECAP ANNUEL</div>
    <h1 className="s1" style={{fontSize:"clamp(28px, 9vw, 52px)",fontWeight:800,color:"white",lineHeight:.88,margin:"10px 0 6px",fontFamily:"Nunito,sans-serif",textShadow:`0 0 120px ${accent}55`}}>
    ...
    <p className="s2" style={{fontSize:16,color:"rgba(255,255,255,.42)",margin:"14px 0 4px",fontFamily:"Nunito,sans-serif"}}>Bienvenue, <span style={{color:"white",fontWeight:600}}>Jérémie</span></p>
    <p className="s3" style={{fontSize:12,color:"rgba(255,255,255,.2)",marginBottom:34,fontFamily:"Nunito,sans-serif"}}>Ton année 2024 · 24 slides<span style={{animation:"blink-c 1s step-end infinite"}}>|</span></p>
    <button className="s4" onClick={onStart} style={{padding:"14px 42px",borderRadius:40,border:"none",cursor:"pointer",background:`linear-gradient(135deg,${accent},#fb923c)`,color:"#000",fontSize:15,fontWeight:800,fontFamily:"Nunito,sans-serif",boxShadow:`0 0 70px ${accent}55`}}>Découvrir →</button>
  </div>
}
function SlideOverview({accent}){
  ...
  return <div style={{maxWidth:430,width:"100%"}}>
    <div className="s0" style={{marginBottom:18}}><Tag accent={accent}/>
      <h2 style={{fontSize:38,fontWeight:800,color:"white",fontFamily:"Nunito,sans-serif",lineHeight:1}}>Ton année<br/><span style={{color:accent}}>en chiffres</span></h2>
    </div>
    <div className="s1" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
      {[{v:1240,s:"h",l:"heures totales"},{v:312,s:"",l:"contenus"},{v:67,s:"",l:"films"},{v:11,s:"",l:"livres audio"}].map((c,i)=>(
        <div key={c.l} className="glass" style={{padding:"14px 16px",animation:`slide-up .5s ease ${.14+i*.09}s both`}}>
          <div style={{fontSize:32,fontWeight:800,color:accent,fontFamily:"Nunito,sans-serif",lineHeight:1,animation:active?`flash-n .8s ease ${.38+i*.1}s both`:"none"}}>{active?<AN t={c.v} s={c.s}/>:`0${c.s}`}</div>
          <Lbl>{c.l}</Lbl>
        </div>
      ))}
    </div>
    ...
    <div className="glass s3" style={{padding:"11px 14px",marginTop:9,display:"flex",alignItems:"center",gap:10}}>
      ...
      <div><div style={{color:"white",fontWeight:700,fontSize:13,fontFamily:"Nunito,sans-serif"}}>Streak record : 47 jours</div><Lbl>Meilleur mois : Décembre</Lbl></div>
  ...
  return <div style={{maxWidth:430,width:"100%"}}>
    <div className="s0" style={{marginBottom:12}}><Tag accent={accent}/><Lbl c={accent} size={9}>🎬 Plex · Films</Lbl>
      <div style={{display:"flex",alignItems:"flex-end",gap:12,marginTop:4}}>
        ...
        <div style={{marginBottom:3}}><div style={{fontSize:22,fontWeight:700,color:`${accent}bb`,fontFamily:"Nunito,sans-serif"}}>{active?<AN t={412} s="h"/>:"0h"}</div><Lbl>visionnés</Lbl></div>
      </div>
...
function SlideFilmsDeep({accent}){
  ...
  return <div style={{maxWidth:430,width:"100%"}}>
    <div className="s0" style={{marginBottom:12}}><Tag accent={accent}/><Lbl c={accent} size={9}>🎬 Films · Habitudes</Lbl>
      <h2 style={{fontSize:26,fontWeight:800,color:"white",fontFamily:"Nunito,sans-serif",lineHeight:1.05,marginTop:4}}>Quand tu regardes<br/><span style={{color:accent}}>des films</span></h2>
    </div>
    <div className="glass s1" style={{padding:"10px 12px",marginBottom:8}}><Lbl c={accent} size={8}>Jour préféré</Lbl><DayChart data={f.dayOfWeek} accent={accent} height={54} unit=" films"/><div style={{fontSize:9,color:"rgba(255,255,255,.25)",fontFamily:"JetBrains Mono,monospace",marginTop:3,textAlign:"right"}}>📅 Peak : samedi</div></div>
    <div className="glass s2" style={{padding:"10px 12px",marginBottom:8}}><Lbl c={accent} size={8}>Heure de démarrage</Lbl><TimeChart data={f.timeOfDay} accent={accent} height={52} unit=" films"/><div style={{fontSize:9,color:"rgba(255,255,255,.25)",fontFamily:"JetBrains Mono,monospace",marginTop:3,textAlign:"right"}}>🌙 Peak : 20h</div></div>
    ...
    <div className="glass s4" style={{padding:"12px 16px"}}>
      <div style={{display:"flex",justifyContent:"space-around"}}>
        {[["2.3","/sem.","Films moy."],["108","min","Durée moy."],["14","revus","Reruns"]].map(([v,suf,l])=>(
          <div key={l} style={{textAlign:"center"}}><div style={{fontFamily:"Nunito,sans-serif",fontWeight:800,fontSize:20,color:accent,lineHeight:1}}>{v}<span style={{fontSize:11,opacity:.7}}>{suf}</span></div><Lbl size={9}>{l}</Lbl></div>
        ))}
      </div>
    </div>
  </div>
  ...
  return <div style={{maxWidth:430,width:"100%"}}>
    <div className="s0" style={{marginBottom:12}}><Tag accent={accent}/><Lbl c={accent} size={9}>🎭 Plex · Séries</Lbl>
      <div style={{display:"flex",alignItems:"flex-end",gap:12,marginTop:4}}>
        ...
        <div style={{marginBottom:3}}><div style={{fontSize:22,fontWeight:700,color:`${accent}bb`,fontFamily:"Nunito,sans-serif"}}>{active?<AN t={435} s="h"/>:"0h"}</div><Lbl>de séries</Lbl></div>
      </div>
...
function SlideSeriesDeep({accent}){
  ...
  return <div style={{maxWidth:430,width:"100%"}}>
    <div className="s0" style={{marginBottom:12}}><Tag accent={accent}/><Lbl c={accent} size={9}>🎭 Séries · Habitudes</Lbl>
      <h2 style={{fontSize:26,fontWeight:800,color:"white",fontFamily:"Nunito,sans-serif",lineHeight:1.05,marginTop:4}}>Quand tu regardes<br/><span style={{color:accent}}>des séries</span></h2>
    </div>
    <div className="glass s1" style={{padding:"10px 12px",marginBottom:8}}><Lbl c={accent} size={8}>Activité par jour</Lbl><DayChart data={s.dayOfWeek} accent={accent} height={58} unit=" épisodes"/><div style={{fontSize:9,color:"rgba(255,255,255,.25)",fontFamily:"JetBrains Mono,monospace",marginTop:3,textAlign:"right"}}>📅 Peak : week-end</div></div>
    <div className="glass s2" style={{padding:"10px 12px",marginBottom:8}}><Lbl c={accent} size={8}>Heure de visionnage</Lbl><TimeChart data={s.timeOfDay} accent={accent} height={52} unit=" épisodes"/><div style={{fontSize:9,color:"rgba(255,255,255,.25)",fontFamily:"JetBrains Mono,monospace",marginTop:3,textAlign:"right"}}>🌙 Peak : 22h — night owl</div></div>
    <div className="glass s3" style={{padding:"12px 16px",marginBottom:8}}>
      <div style={{display:"flex",justifyContent:"space-around"}}>
        {[["3.2","ép/sess.","Session moy."],["8","ép","Binge record"],["4","séries","Terminées"]].map(([v,suf,l])=>(
          <div key={l} style={{textAlign:"center"}}><div style={{fontFamily:"Nunito,sans-serif",fontWeight:800,fontSize:22,color:accent,lineHeight:1}}>{v}<span style={{fontSize:11,opacity:.6}}>{suf}</span></div><Lbl size={9}>{l}</Lbl></div>
        ))}
      </div>
    </div>
  ...
  return <div style={{maxWidth:430,width:"100%"}}>
    <div className="s0" style={{marginBottom:12}}><Tag accent={accent}/><Lbl c={accent} size={9}>🎮 ROMM · Jeux Vidéo</Lbl>
      <div style={{display:"flex",alignItems:"flex-end",gap:14,marginTop:4}}>
        ...
        <div style={{marginBottom:3}}><div style={{fontSize:22,fontWeight:700,color:`${accent}bb`,fontFamily:"Nunito,sans-serif"}}>{active?<AN t={186} s="h"/>:"0h"}</div><Lbl>de jeu</Lbl></div>
      </div>
...
function SlideRommDeep({accent}){
  ...
  return <div style={{maxWidth:430,width:"100%"}}>
    <div className="s0" style={{marginBottom:12}}><Tag accent={accent}/><Lbl c={accent} size={9}>🎮 ROMM · Stats avancées</Lbl>
      <h2 style={{fontSize:26,fontWeight:800,color:"white",fontFamily:"Nunito,sans-serif",lineHeight:1.05,marginTop:4}}>Dans le <span style={{color:accent}}>détail</span></h2>
    </div>
      ...
      <div className="glass" style={{padding:"10px 12px"}}><Lbl c={accent} size={8}>Genres</Lbl>
        <div style={{display:"flex",flexDirection:"column",gap:4,marginTop:6}}>
          {r.genres.map((g,i)=><div key={g.n} style={{display:"flex",justifyContent:"space-between",animation:`slide-up .4s ease ${.3+i*.06}s both`}}>
            <span style={{fontSize:10,color:"rgba(255,255,255,.55)",fontFamily:"Nunito,sans-serif"}}>{g.n}</span>
            <span style={{color:accent,fontSize:10,fontFamily:"JetBrains Mono,monospace"}}>×{g.v}</span>
          </div>)}
        </div>
      </div>
      <div className="glass" style={{padding:"10px 12px"}}><Lbl c={accent} size={8}>Sessions</Lbl>
        <div style={{display:"flex",flexDirection:"column",gap:7,marginTop:8}}>
          {[["4.2h","Durée moy."],["11h","Plus longue"],["67%","Taux compl."]].map(([v,l])=>(
            <div key={l}><div style={{color:accent,fontWeight:800,fontSize:20,fontFamily:"Nunito,sans-serif",lineHeight:1}}>{v}</div><Lbl size={9}>{l}</Lbl></div>
          ))}
        </div>
      </div>
    </div>
  ...
  return <div style={{maxWidth:430,width:"100%"}}>
    <div className="s0" style={{marginBottom:12}}><Tag accent={accent}/><Lbl c={accent} size={9}>🎧 Audiobookshelf</Lbl>
      <div style={{display:"flex",alignItems:"flex-end",gap:12,marginTop:4}}>
        ...
        <div style={{marginBottom:3}}><div style={{fontSize:22,fontWeight:700,color:`${accent}bb`,fontFamily:"Nunito,sans-serif"}}>{active?<AN t={134} s="h"/>:"0h"}</div><Lbl>d'écoute</Lbl></div>
      </div>
...
function SlideAudioDeep({accent}){
  ...
  return <div style={{maxWidth:430,width:"100%"}}>
    <div className="s0" style={{marginBottom:12}}><Tag accent={accent}/><Lbl c={accent} size={9}>🎧 Audio · Habitudes</Lbl>
      <h2 style={{fontSize:26,fontWeight:800,color:"white",fontFamily:"Nunito,sans-serif",lineHeight:1.05,marginTop:4}}>Quand tu<br/><span style={{color:accent}}>écoutes</span></h2>
    </div>
    <div className="glass s1" style={{padding:"10px 12px",marginBottom:8}}><Lbl c={accent} size={8}>Heure d'écoute favorite</Lbl><TimeChart data={a.timeOfDay} accent={accent} height={54} unit="h"/><div style={{fontSize:9,color:"rgba(255,255,255,.25)",fontFamily:"JetBrains Mono,monospace",marginTop:3,textAlign:"right"}}>🌙 Peak : 20h</div></div>
  ...
  return <div style={{maxWidth:430,width:"100%"}}>
    <div className="s0" style={{marginBottom:12}}><Tag accent={accent}/><Lbl c={accent} size={9}>📚 Komga · Lecture</Lbl>
      <div style={{display:"flex",alignItems:"flex-end",gap:12,marginTop:4}}>
        ...
        <div style={{marginBottom:3}}><div style={{fontSize:22,fontWeight:700,color:`${accent}bb`,fontFamily:"Nunito,sans-serif"}}>{active?<AN t={12} s=" séries"/>:"0"}</div><Lbl>commencées</Lbl></div>
      </div>
...
function SlideCompare({accent}){
  return <div style={{maxWidth:430,width:"100%"}}>
    <div className="s0" style={{marginBottom:16}}><Tag accent={accent}/>
      <h2 style={{fontSize:34,fontWeight:800,color:"white",fontFamily:"Nunito,sans-serif",lineHeight:1.05}}>2024 vs <span style={{color:accent}}>2023</span></h2>
    </div>
    <div className="glass s1" style={{padding:"12px 12px",marginBottom:9}}>
      <div style={{display:"flex",gap:12,marginBottom:8}}>{[["2024",accent],["2023","rgba(255,255,255,.25)"]].map(([y,c])=>(
        <div key={y} style={{display:"flex",alignItems:"center",gap:6,fontSize:10,color:c,fontFamily:"JetBrains Mono,monospace"}}><div style={{width:16,height:2.5,background:c,borderRadius:2}}/>{y}</div>
      ))}</div>
    ...
    <div className="glass s2" style={{padding:"12px 14px",marginBottom:9}}>
      <div style={{display:"flex",justifyContent:"space-between"}}>
        {[["🎬","Films",23,"#E5A00D"],["🎭","Séries",19,"#E87C2A"],["🎮","ROMM",8,"#34d399"],["🎧","Audio",45,"#fb923c"],["📚","Komga",34,"#c084fc"]].map(([e,l,v,c])=>(
          <div key={l} style={{textAlign:"center"}}><div style={{fontSize:14}}>{e}</div><div style={{color:c,fontWeight:800,fontSize:16,fontFamily:"Nunito,sans-serif"}}>+{v}%</div><Lbl size={8}>{l}</Lbl></div>
        ))}
      </div>
    </div>
...
function SlideRanking({accent}){
  ...
  return <div style={{maxWidth:430,width:"100%"}}>
    <div className="s0" style={{marginBottom:16}}><Tag accent={accent}/>
      <h2 style={{fontSize:36,fontWeight:800,color:"white",fontFamily:"Nunito,sans-serif",lineHeight:1.0}}>Tu es<br/><span style={{background:`linear-gradient(135deg,${accent},#fb923c)`,WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>#2 sur 6</span></h2>
      <div style={{fontSize:12,color:"rgba(255,255,255,.3)",fontFamily:"Nunito,sans-serif",marginTop:4}}>utilisateurs Xenocloud</div>
    </div>
        ...
        <div key={u.n} style={{display:"flex",alignItems:"center",gap:9,padding:"9px 13px",
          ...
          <div style={{flex:1}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <span style={{fontSize:12,fontFamily:isMe?"Nunito,sans-serif":"Nunito,sans-serif",fontWeight:isMe?800:400,color:isMe?accent:"rgba(255,255,255,.7)"}}>
                {u.n}{isMe&&<span style={{fontSize:9,color:`${accent}70`,fontWeight:400}}> · moi</span>}
              </span>
              <span style={{fontSize:11,color:isMe?accent:"rgba(255,255,255,.3)",fontFamily:"JetBrains Mono,monospace",fontWeight:isMe?700:400}}>{u.v.toLocaleString("fr-FR")}h</span>
            </div>
  ...
  return(
    <div style={{width:"100%",height:"100vh",overflow:"hidden",position:"relative",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",zIndex:10}}>
      ...
      <div style={{position:"relative",zIndex:5,textAlign:"center",maxWidth:400,width:"100%",padding:"0 20px"}}>
        ...
        <div className="s0" style={{fontSize:8,color:"rgba(255,255,255,.2)",letterSpacing:".35em",textTransform:"uppercase",fontFamily:"JetBrains Mono,monospace",marginBottom:10}}>
          MERCI POUR CETTE BELLE ANNÉE
        </div>
        <h1 className="s1" style={{fontSize:38,fontWeight:800,color:"white",lineHeight:1.0,marginBottom:14,fontFamily:"Nunito,sans-serif"}}>
          C'était ton<br/>
        ...
        <div className="s2" style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:6,marginBottom:12}}>
          {[{v:1240,s:"h",l:"Heures totales",c:accent},{v:312,s:"",l:"Contenus",c:"#a78bfa"},{v:4,s:" services",l:"Plateformes",c:"#34d399"},{v:47,s:" jours",l:"Streak max 🔥",c:"#fb923c"}].map((item,i)=>(
            <div key={item.l} style={{padding:"10px 12px",borderRadius:12,
              ...
              <div style={{fontSize:24,fontWeight:800,color:item.c,fontFamily:"Nunito,sans-serif",lineHeight:1}}>
                {active?<AN t={item.v} s={item.s}/>:`0${item.s}`}
              </div>
              <div style={{fontSize:10,color:"rgba(255,255,255,.35)",fontFamily:"Nunito,sans-serif",textTransform:"uppercase",letterSpacing:"0.1em",marginTop:2}}>{item.l}</div>
            </div>
          ))}
        </div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:4}}>
            {[["🎬","847h","Plex","#E5A00D"],["🎮","186h","ROMM","#34d399"],["🎧","134h","Audio","#fb923c"],["📚","89v","Komga","#c084fc"]].map(([e,v,l,c])=>(
              <div key={l} style={{textAlign:"center"}}>
                ...
                <div style={{color:c,fontWeight:800,fontSize:13,fontFamily:"Nunito,sans-serif",marginTop:2}}>{v}</div>
                <div style={{fontSize:8,color:"rgba(255,255,255,.22)",fontFamily:"JetBrains Mono,monospace",textTransform:"uppercase",letterSpacing:".1em"}}>{l}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="s4" style={{display:"flex",justifyContent:"center",marginBottom:16}}>
          <div style={{padding:"8px 20px",borderRadius:40,
            ...
            <span style={{color:accent,fontWeight:800,fontFamily:"Nunito,sans-serif",fontSize:16}}>🥈 #2 Xenocloud </span>
            <span style={{color:"rgba(255,255,255,.4)",fontSize:12,fontFamily:"Nunito,sans-serif"}}>sur 6 utilisateurs</span>
          </div>
        </div>
        ...
        <div className="s5" style={{display:"flex",gap:10,justifyContent:"center"}}>
          <button style={{padding:"12px 28px",borderRadius:40,border:"none",cursor:"pointer",
            ...
            fontFamily:"Nunito,sans-serif",boxShadow:`0 0 60px ${accent}50`}}>📤 Partager</button>
          <button onClick={onRestart} style={{padding:"12px 28px",borderRadius:40,cursor:"pointer",
            background:"transparent",color:"rgba(255,255,255,.35)",fontSize:13,fontFamily:"Nunito,sans-serif",
  ...
  return(
    <div
      ...
      style={{width:"100%",height:"100vh",overflow:"hidden",position:"relative",
        background:curr.bg,transition:"background .75s ease",fontFamily:"Nunito,sans-serif",userSelect:"none"}}>
      ...
      {!isCat&&!isPod&&!isFinale&&slide>0&&slide<SLIDES_DEF.length-1&&(
        <div style={{position:"fixed",bottom:12,left:"50%",transform:"translateX(-50%)",
          color:"rgba(255,255,255,.12)",fontSize:9,fontFamily:"JetBrains Mono,monospace",
