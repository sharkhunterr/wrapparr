...
export default function IntroSlide({ accent, userName, year, onStart, hasComparison }) {
  ...
  return <div style={{ textAlign: "center", maxWidth: "clamp(300px, 80vw, 400px)", width: "100%" }}>
    ...
    <div className="s5" style={{ marginTop: 28, display: "flex", flexWrap: "wrap", justifyContent: "center", gap: 6 }}>
      {hasComparison && (
        <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "6px 12px", borderRadius: "var(--th-radius-pill, 20px)", background: accent + "10", border: "1px solid " + accent + "25", backdropFilter: "blur(8px)", animation: "pulse-hint 2.5s ease-in-out infinite", position: "relative", overflow: "hidden" }}>
          ...
          <span style={{ fontSize: 9, color: "var(--th-text-secondary, rgba(255,255,255,0.45))", position: "relative" }}>
            Compare avec <span style={{ color: accent, fontWeight: 700 }}>{year - 1}</span>
          </span>
        </div>
      )}
      <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "6px 12px", borderRadius: "var(--th-radius-pill, 20px)", background: "var(--th-surface-subtle, rgba(255,255,255,0.04))", border: "1px solid var(--th-border-subtle, rgba(255,255,255,0.08))", backdropFilter: "blur(8px)" }}>
        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="var(--th-text-muted, rgba(255,255,255,0.35))" strokeWidth="2" strokeLinecap="round" style={{ flexShrink: 0 }}><path d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
        <span style={{ fontSize: 9, color: "var(--th-text-muted, rgba(255,255,255,0.35))" }}>
          Change l'annee en haut
        </span>
      </div>
      <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "6px 12px", borderRadius: "var(--th-radius-pill, 20px)", background: "var(--th-surface-subtle, rgba(255,255,255,0.04))", border: "1px solid var(--th-border-subtle, rgba(255,255,255,0.08))", backdropFilter: "blur(8px)" }}>
        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="var(--th-text-muted, rgba(255,255,255,0.35))" strokeWidth="2" strokeLinecap="round" style={{ flexShrink: 0 }}><path d="M5 15l7-7 7 7" /><path d="M5 9l7 7 7-7" /></svg>
        <span style={{ fontSize: 9, color: "var(--th-text-muted, rgba(255,255,255,0.35))" }}>
