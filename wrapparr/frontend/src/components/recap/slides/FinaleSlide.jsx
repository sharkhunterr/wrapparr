    ...
    <div style={{ width: "100%", height: "100vh", overflow: "hidden", position: "relative", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", zIndex: 10 }}>
      ...
      <div ref={contentRef} style={{ position: "relative", zIndex: 5, width: "100%", maxWidth: 500, padding: "20px 16px 40px", overflowY: "auto", maxHeight: "100vh" }}>
        ...
        <div style={{ textAlign: "center", marginBottom: 12 }}>
          ...
          <div style={{ fontSize: 8, color: "rgba(255,255,255,.2)", letterSpacing: ".35em", textTransform: "uppercase", fontFamily: "JetBrains Mono,monospace", marginBottom: 4 }}>WRAPPARR · {year}</div>
          <h1 style={{ fontSize: "clamp(20px, 5vw, 28px)", fontWeight: 800, color: "white", lineHeight: 1.0, marginBottom: 8 }}>
        ...
        {combinedMonthly.some((m) => m.films + m.series > 0) && (
          <div style={{ padding: "8px 10px 4px", borderRadius: 12, background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.15)", backdropFilter: "blur(8px)", marginBottom: 10 }}>
            <div style={{ fontSize: 9, color: "rgba(255,255,255,0.3)", textTransform: "uppercase", letterSpacing: ".1em", marginBottom: 2 }}>Activite mensuelle</div>
            <ResponsiveContainer width="100%" height={100}>
              <AreaChart data={combinedMonthly} margin={{ top: 4, right: 4, left: -10, bottom: 0 }}>
                ...
                <XAxis dataKey="m" tick={{ fill: "rgba(255,255,255,.3)", fontSize: 7 }} axisLine={false} tickLine={false} />
                <YAxis tick={{ fill: "rgba(255,255,255,.15)", fontSize: 6 }} axisLine={false} tickLine={false} width={24} />
        ...
        {(totalFilms > 0 || totalSeries > 0) && (
          <div style={{ display: "flex", gap: 8, marginBottom: 10, position: "relative", overflow: "hidden", borderRadius: 20 }}>
            ...
            {totalFilms > 0 && (
              <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 5, padding: "8px 10px", borderRadius: 20, background: `${accent}15`, border: `1px solid ${accent}35`, backdropFilter: "blur(10px)", position: "relative" }}>
            ...
            {totalSeries > 0 && (
              <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 5, padding: "8px 10px", borderRadius: 20, background: seriesAccent + "15", border: `1px solid ${seriesAccent}35`, backdropFilter: "blur(10px)", position: "relative" }}>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
          ...
          {totalFilms > 0 && (
            <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 6 }}>
              ...
              {/* Top 3 films */}
              {filmsTop.length > 0 && (
                <div style={{ padding: "8px 8px", borderRadius: 10, background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.15)", backdropFilter: "blur(8px)" }}>
                  ...
                  {filmsTop.map((f, i) => (
                    <div key={f.t + i} style={{ display: "flex", alignItems: "center", gap: 5, marginBottom: 5 }}>
                      <span style={{ width: 14, fontSize: 10, fontWeight: 800, color: i === 0 ? accent : "rgba(255,255,255,0.2)", fontFamily: "JetBrains Mono,monospace", textAlign: "center", flexShrink: 0 }}>{i + 1}</span>
                      <MiniPoster src={f.thumb} size={28} />
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: 10, fontWeight: 600, color: i === 0 ? "white" : "rgba(255,255,255,0.5)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", lineHeight: 1.2 }}>{f.t}</div>
                        <div style={{ display: "flex", gap: 4, marginTop: 1, alignItems: "center" }}>
                          {f.plays > 0 && <span style={{ fontSize: 8, color: accent, fontFamily: "JetBrains Mono,monospace" }}>{f.plays}x</span>}
                          {f.y > 0 && <span style={{ fontSize: 8, color: "rgba(255,255,255,0.25)" }}>{f.y}</span>}
                          {f.r > 0 && <span style={{ fontSize: 8, color: "rgba(255,255,255,0.3)" }}>★{f.r}</span>}
                        </div>
                      </div>
                    </div>
                  ))}
              ...
              {filmsGenres.length > 0 && (
                <div style={{ padding: "8px 8px", borderRadius: 10, background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.15)", backdropFilter: "blur(8px)" }}>
                  <div style={{ fontSize: 9, color: "rgba(255,255,255,0.3)", textTransform: "uppercase", letterSpacing: ".08em", marginBottom: 4 }}>Genres</div>
                  {filmsGenres.map((g, i) => (
                    <div key={g.n} style={{ display: "flex", alignItems: "center", gap: 4, marginBottom: 3 }}>
                      <span style={{ width: 44, fontSize: 9, color: i === 0 ? accent : "rgba(255,255,255,0.4)", fontWeight: i === 0 ? 700 : 400, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{g.n}</span>
                      ...
                      <span style={{ fontSize: 9, color: "rgba(255,255,255,0.25)", fontFamily: "JetBrains Mono,monospace", width: 16, textAlign: "right" }}>{g.v}</span>
                    </div>
                  ))}
                </div>
              )}
                ...
                <div style={{ padding: "8px 8px", borderRadius: 10, background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.15)", backdropFilter: "blur(8px)" }}>
                  ...
                  {seriesTop.map((s, i) => (
                    <div key={s.t + i} style={{ display: "flex", alignItems: "center", gap: 5, marginBottom: 5 }}>
                      <span style={{ width: 14, fontSize: 10, fontWeight: 800, color: i === 0 ? seriesAccent : "rgba(255,255,255,0.2)", fontFamily: "JetBrains Mono,monospace", textAlign: "center", flexShrink: 0 }}>{i + 1}</span>
                      <MiniPoster src={s.thumb} size={28} />
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: 10, fontWeight: 600, color: i === 0 ? "white" : "rgba(255,255,255,0.5)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", lineHeight: 1.2 }}>{s.t}</div>
                        <div style={{ display: "flex", gap: 4, marginTop: 1, alignItems: "center" }}>
                          {(s.ep || s.plays) > 0 && <span style={{ fontSize: 8, color: seriesAccent, fontFamily: "JetBrains Mono,monospace" }}>{s.ep || s.plays} ep.</span>}
                          {s.y > 0 && <span style={{ fontSize: 8, color: "rgba(255,255,255,0.25)" }}>{s.y}</span>}
                          {s.r > 0 && <span style={{ fontSize: 8, color: "rgba(255,255,255,0.3)" }}>★{s.r}</span>}
                        </div>
                      </div>
                    </div>
                  ))}
              ...
              {seriesGenres.length > 0 && (
                <div style={{ padding: "8px 8px", borderRadius: 10, background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.15)", backdropFilter: "blur(8px)" }}>
                  ...
                  {seriesGenres.map((g, i) => (
                    <div key={g.n} style={{ display: "flex", alignItems: "center", gap: 4, marginBottom: 3 }}>
                      <span style={{ width: 44, fontSize: 9, color: i === 0 ? seriesAccent : "rgba(255,255,255,0.4)", fontWeight: i === 0 ? 700 : 400, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{g.n}</span>
                      <div style={{ flex: 1, height: 7, background: "rgba(255,255,255,0.05)", borderRadius: 3, overflow: "hidden" }}>
