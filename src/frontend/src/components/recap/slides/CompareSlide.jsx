...
export default function CompareSlide({ accent, comparison, year }) {
  ...
  return <div style={{ maxWidth: "clamp(320px, 85vw, 540px)", width: "100%" }}>
    <div className="s0" style={{ marginBottom: 16 }}><Tag accent={accent} year={year} />
      <h2 style={{ fontSize: "clamp(20px, 6vw, 34px)", fontWeight: 800, color: "var(--th-text)", lineHeight: 1.05 }}>{year} vs <span style={{ color: accent }}>{year - 1}</span></h2>
    </div>
    {monthly.length > 0 && <div className="glass s1" style={{ padding: "12px 12px", marginBottom: 9 }}>
      <div style={{ display: "flex", gap: 12, marginBottom: 8 }}>{[[String(year), accent], [String(year - 1), "var(--th-text-dim)"]].map(([y, c]) => <div key={y} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 10, color: c, fontFamily: "var(--th-font-mono, JetBrains Mono,monospace)" }}><div style={{ width: 16, height: 2.5, background: c, borderRadius: 2 }} />{y}</div>)}</div>
