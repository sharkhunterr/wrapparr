...
export default function PodiumSlide({ accent, bg, data = [], title, icon, jokes = [], statLabel, statKey, statSuffix = "", config = {}, backdrop, serviceType, mediaType = "films" }) {
  const cfg = { ...DEFAULT_CONFIG, ...config }
  const comp = useComparison()
  const L = useLabels()
  // Use comparison data for the correct service and media type
  const prevSvc = comp.active ? (comp.data?.[serviceType] || comp.data?.tautulli || comp.data?.plex || comp.data?.jellyfin || null) : null
  const mediaKey = mediaType === "series" ? "series" : "films"
  const prevTop = prevSvc?.[mediaKey]?.top?.previous || prevSvc?.top?.previous || []
