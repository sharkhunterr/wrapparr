...
export default function UserManagement() {
  ...
  const resetPassword = async (u) => {
    if (!confirm(t("users.resetConfirm").replace("{name}", u.display_name))) return
    try {
  ...
  const removeMapping = async (u, m) => {
    if (!confirm(t("users.deleteLink").replace("{service}", m.service_type).replace("{username}", m.service_username))) return
    try {
      const current = {}
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
        ...
        <h2 style={{ color: "white", fontSize: 17, fontWeight: 700, margin: 0 }}>{t("users.title")}</h2>
      </div>
      <p style={{ color: "rgba(255,255,255,0.35)", fontSize: 12, marginBottom: 16 }}>{t("users.desc")}</p>
      ...
      {showForm ? (
        <div style={{ padding: "14px 16px", borderRadius: 12, background: "rgba(229,160,13,0.04)", border: "1px solid rgba(229,160,13,0.15)", marginBottom: 16 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
            <span style={{ fontSize: 13, fontWeight: 600, color: "#E5A00D" }}>{t("users.newUser")}</span>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 10 }}>
            <input value={form.display_name} onChange={e => setForm({ ...form, display_name: e.target.value })} placeholder={t("common.name")} style={inp} />
            <input value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} placeholder={t("common.email")} type="email" style={inp} />
            <input value={form.password} onChange={e => setForm({ ...form, password: e.target.value })} placeholder={t("common.password")} type="password" style={inp} />
          ...
          return (
            <div key={u.id} style={{ padding: "14px 16px", background: "rgba(255,255,255,0.02)", borderRadius: 12, border: isEditing ? "1px solid rgba(229,160,13,0.2)" : "1px solid rgba(255,255,255,0.05)" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                ...
                <button onClick={() => resetPassword(u)} title={t("users.resetPassword")} style={actionBtn}><KeyRound size={12} /></button>
              </div>
                ...
                <div style={{ marginTop: 8, paddingTop: 8, borderTop: "1px solid rgba(255,255,255,0.04)" }}>
                  {u.mappings?.map(m => (
                    <div key={m.id} style={{ display: "flex", alignItems: "center", gap: 6, padding: "3px 0" }}>
                      ...
                      <button onClick={() => removeMapping(u, m)} title={t("common.delete")} style={{ marginLeft: "auto", background: "none", border: "none", cursor: "pointer", color: "rgba(255,255,255,0.15)", padding: 2 }}><Unlink size={11} /></button>
                    </div>
                  ))}
                  ...
                  {isAddingMap && (
                    <div style={{ display: "flex", gap: 6, marginTop: 4, alignItems: "center" }}>
                      <select value={mappingForm.service_type} onChange={e => {
                        ...
                        <option value="">{t("users.servicePlaceholder")}</option>
                      ...
                      {mappingForm.service_type && serviceUsers[mappingForm.service_type] ? (
                        <select value={mappingForm.service_username} onChange={e => setMappingForm({ ...mappingForm, service_username: e.target.value })} style={{ ...inp, flex: 1, padding: "3px 6px", fontSize: 10 }}>
                          <option value="">{t("users.userPlaceholder")}</option>
                          {serviceUsers[mappingForm.service_type].map(su => <option key={su.id} value={su.name}>{su.name}</option>)}
                        </select>
                      ) : (
                        <input value={mappingForm.service_username} onChange={e => setMappingForm({ ...mappingForm, service_username: e.target.value })} placeholder={t("users.usernamePlaceholder")} style={{ ...inp, flex: 1, padding: "3px 6px", fontSize: 10 }} />
                      )}
              ...
              {!isAddingMap && (
                <button onClick={() => { setAddingMapping(u.id); setMappingForm({ service_type: "", service_username: "" }) }}
                  ...
                  <Plus size={10} /> {t("users.linkService")}
                </button>
              )}
              ...
              {hasTempPw && (
                <div style={{ marginTop: 8, padding: "8px 12px", borderRadius: 8, background: "rgba(229,160,13,0.08)", border: "1px solid rgba(229,160,13,0.2)" }}>
                  <div style={{ fontSize: 9, color: "rgba(255,255,255,0.4)", marginBottom: 2 }}>{t("users.tempPassword")}</div>
                  ...
                  <div style={{ fontSize: 8, color: "rgba(255,255,255,0.2)", marginTop: 2 }}>{t("users.tempPasswordWarn")}</div>
                </div>
              )}
      ...
      {users.length === 0 && (
        <div style={{ textAlign: "center", padding: 48, color: "rgba(255,255,255,0.2)" }}>{t("users.noUsers")}</div>
      )}
    </div>
  )
}
